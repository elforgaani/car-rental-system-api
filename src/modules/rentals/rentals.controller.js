...
export const createRental = async (req, res) => {
    ...
    try {
        ...
        if (!car) {
            return res.status(404).json({ success: true, message: "Car Doesn't Exist" });
        }
        if (car.isRented) {
            return res.status(400).json({ success: false, message: "Car is Already Rented, You Can't Rent it." });
        }
        const rental = await Rental.create(rentalObject);
        const catUpdateResults = await Car.findByIdAndUpdate(rentalObject.carId, { isRented: true });
        res.status(201).json({ success: false, message: "Rental Created Successfully", data: rental });
    } catch (error) {
